...
class StandardPlot {
  ...
  TH1F* getDataHist() { return _hist[kPlotData]; }
		...
		if(higgsLabel.Contains("m_{H},m_{A}")) furtherXOffSet = -0.05;
                ...
                if     (_hist[ic] && isBSMOverlaid) { DrawLegend(xPos[j], 0.84 - yOff[j]*_yoffset*1.10, _hist[ic], higgs2Label, "f" ); j++; }
                else if(_hist[ic])		    { DrawLegend(xPos[j], 0.84 - yOff[j]*_yoffset*1.10, _hist[ic], higgs2Label, "l" ); j++; }
	      }
	      else {
