...
class StandardPlot_ftr21001 {

    public: 
        StandardPlot_ftr21001() { _doApplyBinWidth = false; _hist.resize(nPlotCategories,0); _hist[kPlotData] = 0; _breakdown = false; _HiggsLabel = ""; _Higgs2Label = "";_labelEM = " Nonprompt";_labelVVV = " VVV";}
  ...
  TH1F* getDataHist() { return _hist[kPlotData]; }
		...
		if(higgs2Label.Contains("ADD")) furtherXOffSet = -0.05;
                if     (_hist[ic] && isBSMOverlaid) { DrawLegend(xPos[j]+furtherXOffSet, 0.84 - yOff[j]*_yoffset*1.0, _hist[ic], higgs2Label, "f" ); j++; }
                else if(_hist[ic])		    { DrawLegend(xPos[j]+furtherXOffSet, 0.84 - yOff[j]*_yoffset*1.0, _hist[ic], higgs2Label, "l" ); j++; }
	      }
