...
#include "StandardPlot_exo19003.C"
#include "GoodStyle.C"
...
void finalPlot_exo19003(int nsel = 0, int ReBin = 1, TString XTitle = "N_{jets}", TString units = "", TString plotName = "histoWW_56.root", TString outputName = "njets",
  ...
  GoodStyle();
  //gROOT->LoadMacro("StandardPlot_exo19003.C");
  gStyle->SetOptStat(0);

  TH1F* _hist[nPlotCategories];
  StandardPlot_exo19003 myPlot;
...
//printf("%d %d %d\n",hDataDivision->GetNbinsX(),hTotalDivision->GetNbinsX(),gd.GetN () );
  ...
  minRatio = TMath::Min(TMath::Max(minRatio-0.10,0.000),0.001);
