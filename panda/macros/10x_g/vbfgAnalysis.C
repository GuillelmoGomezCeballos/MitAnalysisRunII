...
(thePandaFlat.runNumber==306029&&thePandaFlat.eventNumber==31771161);
	...
	if(theMinSelType != ESEL) rndWeights[5] = 1-gRandom->Uniform()*0.02;
	  ...
	  if(dataCardSel >= 0) {
	    ...
	    histo_Baseline[theCategory]->Fill(MVAVar,totalWeight);
	    double qcd_error = 1;
	    if(theCategory == kPlotDY) qcd_error = 1.2;
	    histo_QCDScaleBounding[theCategory][0]->Fill(MVAVar,totalWeight*TMath::Abs(thePandaFlat.scale[0])*qcd_error/maxQCDscale);
	    histo_QCDScaleBounding[theCategory][1]->Fill(MVAVar,totalWeight*TMath::Abs(thePandaFlat.scale[1])*qcd_error/maxQCDscale);
	    histo_QCDScaleBounding[theCategory][2]->Fill(MVAVar,totalWeight*TMath::Abs(thePandaFlat.scale[2])*qcd_error/maxQCDscale);
	    histo_QCDScaleBounding[theCategory][3]->Fill(MVAVar,totalWeight*TMath::Abs(thePandaFlat.scale[3])*qcd_error/maxQCDscale);
	    histo_QCDScaleBounding[theCategory][4]->Fill(MVAVar,totalWeight*TMath::Abs(thePandaFlat.scale[4])*qcd_error/maxQCDscale);
	    histo_QCDScaleBounding[theCategory][5]->Fill(MVAVar,totalWeight*TMath::Abs(thePandaFlat.scale[5])*qcd_error/maxQCDscale);
