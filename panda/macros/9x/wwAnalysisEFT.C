...
int year
  ...
  for(int thePlot=0; thePlot<allPlots; thePlot++){
    ...
    else if(thePlot >=  54 && thePlot <=  71) {nBinPlot = 200; xminPlot =  0.0; xmaxPlot = 200;}
    else if(thePlot >=  72 && thePlot <=  80) {nBinPlot =   3; xminPlot = -0.5; xmaxPlot = 2.5;}
    else if(thePlot >=  81 && thePlot <=  89) {nBinPlot =   3; xminPlot = -0.5; xmaxPlot = 2.5;}
  ...
  for(UInt_t ifile=0; ifile<infileName_.size(); ifile++) {
	...
	qTot = qTot + abs(looseLepPdgId[i])/looseLepPdgId[i];
      ...
      bool passZVeto = dilep.M() > 20;
      bool passPTLLCut = dilep.Pt() > 30;
      bool passMET     = minPMET     > 20;
      bool passMETUp   = minPMETUp   > 20;
      bool passMETDown = minPMETDown > 20;

      bool passNjets     = thePandaFlat.nJot              <= 20;
      bool passNjetsUp   = thePandaFlat.nJot_JESTotalUp   <= 20;
      bool passNjetsDown = thePandaFlat.nJot_JESTotalDown <= 20;
	...
	double combVar = typeSelAux[0] + nBinWWMLL*typeSelAux[1] + nBinWWMLL*nBinWWDRLL*typeSelAux[2];
      ...
      float nGenJets = 0;
      if(thePandaFlat.genJet1Pt > 30 && TMath::Abs(thePandaFlat.genJet1Eta) < 4.7) nGenJets++;
      if(thePandaFlat.genJet2Pt > 30 && TMath::Abs(thePandaFlat.genJet2Eta) < 4.7) nGenJets++;
      ...
      if(passNoBtagSel)histo[lepType+ 72][theCategory]->Fill(nGenJets,totalWeight);
      if(passWWSel)    histo[lepType+ 81][theCategory]->Fill(nGenJets,totalWeight);
