...
int year, int fidAna = 0, TString wwPath = "wwframe", bool useTwoBDTs = true, int mHVal = 0, TString WZName = "WZ3l_MG"
  ...
  double jetPtCut = 50;
  TString fidAnaName = "";
  if     (fidAna == 6) {fidAnaName = Form("_fiducial%d_mH%d",fidAna,mHVal); jetPtCut = 30.; syst_fake[0] = 0.2; syst_fake[1] = 0.2;}
  ...
  const int nBinHT = 4; Float_t xbinsHT[nBinHT+1] = {0,1.5,3,5,1000.};
...
*/
  ...
  else if(fidAna == 6) nBinMVAAux = 6*nBinHT;
  ...
  for(UInt_t ifile=0; ifile<infileName_.size(); ifile++) {
	...
	if(TMath::Abs(thePandaFlat.jotEta[1]) > 2.4) passJetPtEtaCut = passJetPtEtaCut && thePandaFlat.jotPt[1] > jetPtCut;
      ...
      if(thePandaFlat.nLooseMuon < 2) continue;
	...
	if(TMath::Abs(vJot2.Eta()) > 2.4) {
      ...
      double sumHTRat        = (vJot1.Pt()       +vJot2.Pt()       )/vZ1l1.Pt();
      double sumHTRatJESUp   = (vJot1JESUp.Pt()  +vJot2JESUp.Pt()  )/vZ1l1.Pt();
      double sumHTRatJESDown = (vJot1JESDown.Pt()+vJot2JESDown.Pt())/vZ1l1.Pt();
      double sumHTRatJERUp   = (vJot1JERUp.Pt()  +vJot2JERUp.Pt()  )/vZ1l1.Pt();
      double sumHTRatJERDown = (vJot1JERDown.Pt()+vJot2JERDown.Pt())/vZ1l1.Pt();
      bool passMJJ        = thePandaFlat.nJot              >= 2 && massJJ        > 750 && passJetPtEtaCut;
      bool passMJJJESUp   = thePandaFlat.nJot_JESTotalUp   >= 2 && massJJJESUp   > 750 && passJetPtEtaSyst[0];
      bool passMJJJESDown = thePandaFlat.nJot_JESTotalDown >= 2 && massJJJESDown > 750 && passJetPtEtaSyst[1];
      bool passMJJJERUp   = thePandaFlat.nJot              >= 2 && massJJJERUp   > 750 && passJetPtEtaSyst[2];
      bool passMJJJERDown = thePandaFlat.nJot              >= 2 && massJJJERDown > 750 && passJetPtEtaSyst[3];
      ...
      bool passCRMJJ         = thePandaFlat.nJot              >= 2 && massJJ        > 200 && massJJ	   <= 750 && passJetPtEtaCut;
      bool passCRMJJJESUp    = thePandaFlat.nJot_JESTotalUp   >= 2 && massJJJESUp   > 200 && massJJJESUp   <= 750 && passJetPtEtaSyst[0];
      bool passCRMJJJESDown  = thePandaFlat.nJot_JESTotalDown >= 2 && massJJJESDown > 200 && massJJJESDown <= 750 && passJetPtEtaSyst[1];
      bool passCRMJJJERUp    = thePandaFlat.nJot              >= 2 && massJJJERUp   > 200 && massJJJERUp   <= 750 && passJetPtEtaSyst[2];
      bool passCRMJJJERDown  = thePandaFlat.nJot              >= 2 && massJJJERDown > 200 && massJJJERDown <= 750 && passJetPtEtaSyst[3];
	...
	vLoose[0].Pt(),vLoose[0].Eta(),vLoose[0].Phi(),vLoose[1].Pt(),vLoose[1].Eta(),vLoose[1].Phi(),vLoose[2].Pt(),vLoose[2].Eta(),vLoose[2].Phi(),bdtWZValue);
	  ...
	  for(int ib=0; ib<nBinMJJCR; ib++){
	     if(TMath::Min(massJJJERDown,xbinsMJJCR[nBinMJJCR]-0.001) < xbinsMJJCR[ib+1]) {typeSelAux1CR[4] = ib; break;}
          ...
          double typeSelHT[5] = {0,0,0,0,0};
	  for(int ib=0; ib<nBinHT; ib++){
	     if(TMath::Min(sumHTRat       ,xbinsHT[nBinHT]-0.001) < xbinsHT[ib+1]) {typeSelHT[0] = ib; break;}
	  }
	  for(int ib=0; ib<nBinHT; ib++){
	     if(TMath::Min(sumHTRatJESUp  ,xbinsHT[nBinHT]-0.001) < xbinsHT[ib+1]) {typeSelHT[1] = ib; break;}
	  }
	  for(int ib=0; ib<nBinHT; ib++){
	     if(TMath::Min(sumHTRatJESDown,xbinsHT[nBinHT]-0.001) < xbinsHT[ib+1]) {typeSelHT[2] = ib; break;}
	  }
	  for(int ib=0; ib<nBinHT; ib++){
	     if(TMath::Min(sumHTRatJERUp  ,xbinsHT[nBinHT]-0.001) < xbinsHT[ib+1]) {typeSelHT[3] = ib; break;}
	  }
	  for(int ib=0; ib<nBinHT; ib++){
	     if(TMath::Min(sumHTRatJERDown,xbinsHT[nBinHT]-0.001) < xbinsHT[ib+1]) {typeSelHT[4] = ib; break;}
	  }
          double typeSelAux0[5] = {0,0,0,0,0};
	  ...
	  if(massJJJERDown > 1500) typeSelAux1[4] += nBinAQGCMTWZ;
          ...
          int whichBin = 0;
          if     (dataCardSel        == 0) MVAVar        = typeSelHT[whichBin]  + nBinHT*(dphill>2);
          else if(dataCardSel        == 4) MVAVar        = 2*nBinHT + typeSelHT[whichBin];
          else if(dataCardSel        == 1) MVAVar        = 3*nBinHT + typeSelHT[whichBin];
          else if(dataCardSel        == 3) MVAVar        = 4*nBinHT + typeSelHT[whichBin];
          else if(dataCardSel        == 2) MVAVar        = 5*nBinHT + typeSelHT[whichBin];

          whichBin = 1;
          if     (dataCardSelJESUp   == 0) MVAVarJESUp   = typeSelHT[whichBin]  + nBinHT*(dphill>2);
          else if(dataCardSelJESUp   == 4) MVAVarJESUp   = 2*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJESUp   == 1) MVAVarJESUp   = 3*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJESUp   == 3) MVAVarJESUp   = 4*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJESUp   == 2) MVAVarJESUp   = 5*nBinHT + typeSelHT[whichBin];

          whichBin = 2;
          if     (dataCardSelJESDown == 0) MVAVarJESDown = typeSelHT[whichBin]  + nBinHT*(dphill>2);
          else if(dataCardSelJESDown == 4) MVAVarJESDown = 2*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJESDown == 1) MVAVarJESDown = 3*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJESDown == 3) MVAVarJESDown = 4*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJESDown == 2) MVAVarJESDown = 5*nBinHT + typeSelHT[whichBin];

          whichBin = 3;
          if     (dataCardSelJERUp   == 0) MVAVarJERUp   =typeSelHT[whichBin]  + nBinHT*(dphill>2);
          else if(dataCardSelJERUp   == 4) MVAVarJERUp   =2*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJERUp   == 1) MVAVarJERUp   =3*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJERUp   == 3) MVAVarJERUp   =4*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJERUp   == 2) MVAVarJERUp   =5*nBinHT + typeSelHT[whichBin];

          whichBin = 4;
          if     (dataCardSelJERDown == 0) MVAVarJERDown = typeSelHT[whichBin]  + nBinHT*(dphill>2);
          else if(dataCardSelJERDown == 4) MVAVarJERDown = 2*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJERDown == 1) MVAVarJERDown = 3*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJERDown == 3) MVAVarJERDown = 4*nBinHT + typeSelHT[whichBin];
          else if(dataCardSelJERDown == 2) MVAVarJERDown = 5*nBinHT + typeSelHT[whichBin];
        }
        else if(fidAna == 7){
          double typeSelAux0 = 0;
  ...
  for(unsigned ic=0; ic<nPlotCategories; ic++) {
    ...
    else if(fidAna == 6 && (ic == kPlotSignal1 || ic == kPlotBSM || ic == kPlotWZ || ic == kPlotEWKSSWW)) {
      histo_QCDscaleUp   [ic]->Scale(histo_Baseline[ic]->GetSumOfWeights()/histo_QCDscaleUp   [ic]->GetSumOfWeights());
      histo_QCDscaleDown [ic]->Scale(histo_Baseline[ic]->GetSumOfWeights()/histo_QCDscaleDown [ic]->GetSumOfWeights());
    }
